...
class SARSA_Agent(HW2Agent):
    ...
    def Q(self, s):
        eps = self.epsilon(self.iteration)
        
        if random.random() > eps:
            # Choose the action with highest predicted return
            return np.random.choice(
                    np.where(
                        self.Q_arr[s] == np.max(self.Q_arr[s])
                    )[0]
                )
    ...
    def episode(self, start_state=None):
        ...
        while self.states[s] != goal and step < self.max_steps:
            ...
            self.Q_arr[s][a] = (
                ...
                * (r + self.gamma*self.Q_arr[s_prime][a_prime] - self.Q_arr[s][a])
            )
    ...
    def run(self, num_episodes, rand_start=False):
        avg_reward = []
